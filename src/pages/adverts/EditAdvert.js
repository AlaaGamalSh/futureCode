...
import { editAdverts, getStors } from "../../redux/services/adverts_service";
...
export default function EditAdvert() {
  ...
  const { isLoadingUpdateAdverts, stors } = useSelector(
    (state) => state.adverts
  );
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      dispatch(editAdverts(adForm)).then((response) => {
  ...
  return (
    <>
      <Helmet>
        <title>{translate("edit_advert")}</title>
      </Helmet>

      <Container maxWidth={themeStretch ? false : "xl"}>
        <Box sx={boxTitleSx}>
          <Typography sx={titleTypographySx}>
            {translate(`edit_advert`)}
          </Typography>
        </Box>
        ...
        <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
          ...
          <Box sx={boxButtonSx}>
            <LoadingButton
              ...
              type="submit"
              variant="Contained"
              loading={isLoadingUpdateAdverts}
